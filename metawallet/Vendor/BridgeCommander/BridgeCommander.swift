...
public class BridgeCommander : NSObject, WKScriptMessageHandler {
    ...
    let bridgeScriptObject = "iosJsBridge"
    ...
    func parse(_ body: Any) -> BridgeMessage? {
        do {
            ...
            if message.args == nil, let intArgs = parsedData["args"] as? Int {
                message.args = String(intArgs)
            }
            return message
...
public class BridgeCommand: NSObject {
